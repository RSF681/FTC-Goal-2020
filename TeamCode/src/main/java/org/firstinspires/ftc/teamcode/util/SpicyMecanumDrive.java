...
/**
 * ============================================
 *  MOTOR IS FUCKED BLACK TEAM EDITION
 * ============================================
 ...
 */
public class SpicyMecanumDrive extends RobotDrive {
    private double rightSideMultiplier;

    Motor[] motors;

    private double nonShitMotorMult;
     ...
     */
    public SpicyMecanumDrive(Motor frontLeft, Motor frontRight, Motor backLeft, Motor backRight, double nonShitMotorMult) {
        this(true, frontLeft, frontRight, backLeft, backRight, nonShitMotorMult);
     ...
     * @param backRight     the back right motor
     */
    public SpicyMecanumDrive(boolean autoInvert, Motor frontLeft, Motor frontRight, Motor backLeft, Motor backRight, double nonShitMotorMult) {
        motors = new Motor[]{frontLeft, frontRight, backLeft, backRight};
        this.nonShitMotorMult = nonShitMotorMult;
    ...
    public void driveFieldCentric(double strafeSpeed, double forwardSpeed,
        ...
        // Back left motor is 312 rpm, everything else is 435 rpm
        motors[MotorType.kFrontLeft.value]
                .set(wheelSpeeds[MotorType.kFrontLeft.value] * maxOutput * nonShitMotorMult);
        motors[MotorType.kFrontRight.value]
                .set(wheelSpeeds[MotorType.kFrontRight.value] * rightSideMultiplier * maxOutput * nonShitMotorMult);
        motors[MotorType.kBackLeft.value]
                .set(wheelSpeeds[MotorType.kBackLeft.value] * maxOutput);
        motors[MotorType.kBackRight.value]
                .set(wheelSpeeds[MotorType.kBackRight.value] * rightSideMultiplier * maxOutput * nonShitMotorMult);
    }
